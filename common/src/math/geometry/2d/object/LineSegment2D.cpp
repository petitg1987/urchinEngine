...
namespace urchin {
    ...
    template<class T> T LineSegment2D<T>::squareDistance(const Point2<T>& p) const {
        ...
        if (typeid(int) == typeid(T) || typeid(long) == typeid(T) || typeid(long long) == typeid(T)) {
            return ap.squareLength() - MathFunction::instance()->roundDivision<T>(apDotAb * apDotAb, abSquareLength);
        }
    ...
    template<class T> Point2<T> LineSegment2D<T>::intersectPoint(const LineSegment2D<T>& other, bool& hasIntersection,
        ...
        if ( //if (t >= 0 && t <= 1 && u >= 0 && u <= 1) where t = thisToOtherCrossR / rCrossS and u = startPointsCrossR / rCrossS
                (thisToOtherCrossR == T(0) || MathFunction::instance()->sign<T>(thisToOtherCrossR) == MathFunction::instance()->sign<T>(rCrossS))
                && std::abs(rCrossS) >= std::abs(thisToOtherCrossR)
                && (startPointsCrossR == T(0) || MathFunction::instance()->sign<T>(startPointsCrossR) == MathFunction::instance()->sign<T>(rCrossS))
                && std::abs(rCrossS) >= std::abs(startPointsCrossR)) { //intersection
            hasIntersection = true;
            if (typeid(int) == typeid(T) || typeid(long) == typeid(T) || typeid(long long) == typeid(T)) {
                Vector2<T> vTranslate(MathFunction::instance()->roundDivision<T>(thisToOtherCrossR * r.X, rCrossS),
                                      MathFunction::instance()->roundDivision<T>(thisToOtherCrossR * r.Y, rCrossS));
