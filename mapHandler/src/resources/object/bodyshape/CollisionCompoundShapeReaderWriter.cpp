...
namespace urchin {
    ...
    CollisionShape3D* CollisionCompoundShapeReaderWriter::loadFrom(const DataChunk* mainShapeChunk, const DataParser& dataParser) const {
        ...
        auto localizedShapesChunk = dataParser.getChunks(LOCALIZED_SHAPE, DataAttribute(), localizedShapesListChunk);
        ...
        for (std::size_t i = 0; i < localizedShapesChunk.size(); ++i) {
            ...
            auto shapeChunk = dataParser.getUniqueChunk(true, COMPOUND_SHAPE_TAG, DataAttribute(), localizedShapesChunk[i]);
    ...
    void CollisionCompoundShapeReaderWriter::writeOn(DataChunk& mainShapeChunk, const CollisionShape3D& mainCollisionShape, DataWriter& dataWriter) const {
        ...
        for (const auto& localizedShape : localizedShapes) {
            auto& localizedShapeChunk = dataWriter.createChunk(LOCALIZED_SHAPE, DataAttribute(), &localizedShapesListChunk);

            writeTransformOn(localizedShapeChunk, *localizedShape, dataWriter);

            auto& shapeChunk = dataWriter.createChunk(COMPOUND_SHAPE_TAG, DataAttribute(), &localizedShapeChunk);
            CollisionShapeReaderWriterRetriever::retrieveShapeReaderWriter(*localizedShape->shape)->writeOn(shapeChunk, *localizedShape->shape, dataWriter);
        }
    }
    ...
    void CollisionCompoundShapeReaderWriter::writeTransformOn(DataChunk& localizedShapeChunk, const LocalizedCollisionShape& localizedShape, DataWriter& dataWriter) {
        ...
        OrientationReaderWriter::writeOrientation(transformChunk, localizedShape.transform.getOrientation(), dataWriter);
    }
