...
void CollisionWorldIT::fallOnGround() {
    auto bodyContainer = buildWorld(Point3(0.0f, 5.0f, 0.0f));
...
void CollisionWorldIT::ccdPushOnGround() {
    auto bodyContainer = buildWorld(Point3(0.0f, 5.0f, 0.0f));
    auto collisionWorld = std::make_unique<CollisionWorld>(*bodyContainer);
    collisionWorld->process(1.0f / 1000.0f, Vector3(0.0f, 0.0f, 0.0f));
    auto* cubeBody = static_cast<RigidBody*>(bodyContainer->getBodies()[1].get());
    cubeBody->applyCentralMomentum(Vector3(0.0f, -1000.0f, 0.0f)); //apply extreme down force
    cubeBody->setLinearFactor(Vector3(0.0f, 1.0f, 0.0f)); //avoid cube to go on outside world border
    cubeBody->setAngularFactor(Vector3(0.0f, 0.0f, 0.0f)); //avoid cube to go on outside world border
...
void CollisionWorldIT::ccdBounceOnGroundAndRoof() {
    auto bodyContainer = buildWorld(Point3(0.0f, 5.0f, 0.0f));
    auto roofShape = std::make_unique<CollisionBoxShape>(Vector3(50.0f, 0.5f, 50.0f));
    bodyContainer->addBody(std::make_unique<RigidBody>("roof", PhysicsTransform(Point3(0.0f, 10.0f, 0.0f), Quaternion<float>()), std::move(roofShape)));
    auto collisionWorld = std::make_unique<CollisionWorld>(*bodyContainer);
    collisionWorld->process(1.0f / 1000.0f, Vector3(0.0f, 0.0f, 0.0f));
    auto* cubeBody = static_cast<RigidBody*>(bodyContainer->getBodies()[1].get());
    cubeBody->applyCentralMomentum(Vector3(0.0f, -10000.0f, 0.0f)); //apply extreme down force
    ...
    collisionWorld->process(1.0f / 30.0f, Vector3(0.0f, -9.81f, 0.0f));
...
void CollisionWorldIT::fallForever() {
    ...
    auto bodyContainer = buildWorld(Point3(60.0f, 5.0f, 0.0f));
    auto collisionWorld = std::make_unique<CollisionWorld>(*bodyContainer);
...
void CollisionWorldIT::changeMass() {
    auto bodyContainer = buildWorld(Point3(0.0f, 10.0f, 0.0f));
    auto collisionWorld = std::make_unique<CollisionWorld>(*bodyContainer);
...
std::unique_ptr<BodyContainer> CollisionWorldIT::buildWorld(const Point3<float>& cubePosition) const {
    ...
    auto groundShape = std::make_unique<CollisionBoxShape>(Vector3(50.0f, 0.5f, 50.0f));
    auto groundBody = std::make_unique<RigidBody>("ground", PhysicsTransform(Point3(0.0f, -0.5f, 0.0f), Quaternion<float>()), std::move(groundShape));
    bodyContainer->addBody(std::move(groundBody));

    auto cubeShape = std::make_unique<CollisionBoxShape>(Vector3(0.5f, 0.5f, 0.5f));
