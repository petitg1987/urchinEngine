...
namespace urchin {
    ...
    template<class T> std::unique_ptr<ContinuousCollisionResult<T>, AlgorithmResultDeleter> AlgorithmResultAllocator::newContinuousCollisionResult(std::shared_ptr<AbstractBody> body2, const Vector3<T>& normalFromObject2, const Point3<T>& hitPointOnObject2, T timeToHit) {
        void* memPtr = AlgorithmResultAllocator::instance().getAlgorithmResultPool().allocate(sizeof(ContinuousCollisionResult<T>));
        return std::unique_ptr<ContinuousCollisionResult<T>, AlgorithmResultDeleter>(new(memPtr) ContinuousCollisionResult<T>(std::move(body2), normalFromObject2, hitPointOnObject2, timeToHit), AlgorithmResultDeleter());
    }
    ...
    template class std::unique_ptr<ContinuousCollisionResult<float>, AlgorithmResultDeleter> AlgorithmResultAllocator::newContinuousCollisionResult(std::shared_ptr<AbstractBody> body2, const Vector3<float>& normalFromObject2, const Point3<float>& hitPointOnObject2, float timeToHit);
    ...
    template class std::unique_ptr<ContinuousCollisionResult<double>, AlgorithmResultDeleter> AlgorithmResultAllocator::newContinuousCollisionResult(std::shared_ptr<AbstractBody> body2, const Vector3<double>& normalFromObject2, const Point3<double>& hitPointOnObject2, double timeToHit);
}
