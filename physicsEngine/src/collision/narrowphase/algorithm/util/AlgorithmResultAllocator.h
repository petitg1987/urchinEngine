...
namespace urchin {
    ...
    class AlgorithmResultAllocator : public ThreadSafeSingleton<AlgorithmResultAllocator> {
        public:
            ...
            template<class T> std::unique_ptr<ContinuousCollisionResult<T>, AlgorithmResultDeleter> newContinuousCollisionResult(std::shared_ptr<AbstractBody>, const Vector3<T>&, const Point3<T>&, T);
