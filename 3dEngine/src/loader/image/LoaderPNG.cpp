...
namespace urchin {
    ...
    std::vector<uint16_t> LoaderPNG::extract16BitsChannels(const std::vector<unsigned char>& pixelsRGBA16bits, unsigned int channelsMask) {
        ...
        for (std::size_t i=7; i<pixelsRGBA16bits.size(); i+=8) {
            if (channelsMask & 1u) { //red
                pixels.push_back(static_cast<uint16_t>(static_cast<uint16_t>(pixelsRGBA16bits[i-7] << 8u) | static_cast<uint16_t>(pixelsRGBA16bits[i-6])));
            }
            if (channelsMask & 2u) { //green
                pixels.push_back(static_cast<uint16_t>(static_cast<uint16_t>(pixelsRGBA16bits[i-5] << 8u) | static_cast<uint16_t>(pixelsRGBA16bits[i-4])));
            }
            if (channelsMask & 4u) { //blue
                pixels.push_back(static_cast<uint16_t>(static_cast<uint16_t>(pixelsRGBA16bits[i-3] << 8u) | static_cast<uint16_t>(pixelsRGBA16bits[i-2])));
            }
            if (channelsMask & 8u) { //alpha
                pixels.push_back(static_cast<uint16_t>(static_cast<uint16_t>(pixelsRGBA16bits[i-1] << 8u) | static_cast<uint16_t>(pixelsRGBA16bits[i-0])));
            }
        }
