    ...
    std::shared_ptr<Texture> LoaderTexture::loadFromFile(const std::string& filename, const std::map<std::string, std::string, std::less<>>& params) {
        if (filename == DEFAULT_TEXTURE_FILENAME) {
            ...
            if (defaultTextureType == "normal") {
                ...
                return Texture::build("defaultNormal", 1, 1, TextureFormat::RGBA_8_UINT_NORM, defaultNormalColor.data(), TransparencyData::buildOpaque(), TextureDataType::INT_8);
            } else if (defaultTextureType == "roughness") {
                std::vector<unsigned char> defaultRoughnessValue({255});
                return Texture::build("defaultRoughness", 1, 1, TextureFormat::GRAYSCALE_8_UINT_NORM, defaultRoughnessValue.data(), TransparencyData::buildOpaque(), TextureDataType::INT_8);
            } else if (defaultTextureType == "metalness") {
                std::vector<unsigned char> defaultMetalnessValue({0});
                return Texture::build("defaultMetalness", 1, 1, TextureFormat::GRAYSCALE_8_UINT_NORM, defaultMetalnessValue.data(), TransparencyData::buildOpaque(), TextureDataType::INT_8);
            } else {
        ...
        if (image->getChannelPrecision() == Image::CHANNEL_8_INT) {
            texture = Texture::build(image->getName(), image->getWidth(), image->getHeight(), image->retrieveTextureFormat(), image->getTexels().data(), image->getTransparencyData(), TextureDataType::INT_8);
        }else if (image->getChannelPrecision() == Image::CHANNEL_16_INT) {
            texture = Texture::build(image->getName(), image->getWidth(), image->getHeight(), image->retrieveTextureFormat(), image->getTexels16Bits().data(), image->getTransparencyData(), TextureDataType::INT_16);
        } else {
