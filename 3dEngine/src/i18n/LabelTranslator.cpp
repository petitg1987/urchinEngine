...
namespace urchin {
        ...
        for (unsigned int i = 0; i < availableLanguages.size(); ++i) {
            ...
            for (const auto& [firstLabelKey, firstLabelValue] : firstLanguageLabels) {
                ...
                if (firstLabelValue.empty() != secondLabelValue.empty()) {
                    ...
                    Logger::instance().logError("Translation value for label '" + firstLabelKey + "' are different on curly bracket for languages '" + firstLanguage + "' and '" + secondLanguage + "'");
                } else if (std::ranges::count(firstLabelValue, '}') != std::ranges::count(secondLabelValue, '}')) {
                    Logger::instance().logError("Translation value for label '" + firstLabelKey + "' are different on curly bracket for languages '" + firstLanguage + "' and '" + secondLanguage + "'");
                } else if (isFirstLetterUpper(firstLabelValue) != isFirstLetterUpper(secondLabelValue)) {
    ...
    bool LabelTranslator::isFirstLetterUpper(std::string_view sentence) const {
        const char* textIt = &sentence[0];
        const char* textEndIt = textIt + sentence.size();
        while (textIt != textEndIt) {
            char32_t firstLEtterCp = StringUtil::readNextCodepoint(textIt, textEndIt);

            if (firstLEtterCp == L'À' || firstLEtterCp == L'Ç' || firstLEtterCp == L'É' || firstLEtterCp == L'È' || firstLEtterCp == L'Ê' || firstLEtterCp == L'Ù' || firstLEtterCp == L'Á') {
                return true;
            }

            if (firstLEtterCp <= std::numeric_limits<char>::max()) {
                char char8 = static_cast<char>(firstLEtterCp);
