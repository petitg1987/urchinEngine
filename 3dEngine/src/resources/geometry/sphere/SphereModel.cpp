        ...
        for (const Sphere<float>& sphere : spheres) {
            ...
            for (unsigned int i = 1; i <= slices; i++) {
                ...
                for (unsigned int j = 0; j < slices; j++) {
                    ...
                    vertexArray.emplace_back(sphere.getCenterOfMass() + Point3(x1 * zr0 * radius, y1 * zr0 * radius, z0 * radius));
                    vertexArray.emplace_back(sphere.getCenterOfMass() + Point3(x1 * zr1 * radius, y1 * zr1 * radius, z1 * radius));
                    vertexArray.emplace_back(sphere.getCenterOfMass() + Point3(x2 * zr1 * radius, y2 * zr1 * radius, z1 * radius));

                    vertexArray.emplace_back(sphere.getCenterOfMass() + Point3(x1 * zr0 * radius, y1 * zr0 * radius, z0 * radius));
                    vertexArray.emplace_back(sphere.getCenterOfMass() + Point3(x2 * zr1 * radius, y2 * zr1 * radius, z1 * radius));
                    vertexArray.emplace_back(sphere.getCenterOfMass() + Point3(x2 * zr0 * radius, y2 * zr0 * radius, z0 * radius));
                }
            }
        }
