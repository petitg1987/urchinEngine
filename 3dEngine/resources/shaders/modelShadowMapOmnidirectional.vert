...
layout(std140, set = 0, binding = 2) uniform ShadowMatrixData {
    mat4 lightProjectionViewMatrices[NUMBER_SHADOW_MAPS];
} shadowMatrixData;

layout(std140, set = 0, binding = 3) uniform ShadowScopeData {
...
void main() {
    vec4 position = shadowMatrixData.lightProjectionViewMatrices[pushConstants.layerIndex] * (mModel * vec4(vertexPosition, 1.0));

    //depth linearization could be done directly via "shadowData.lightProjectionMatrix" but I didn't find how to do it !
    position.z = linearizeDepth(position.z / position.w, shadowScopeData.omnidirectionalNearPlane, shadowScopeData.omnidirectionalFarPlane) * position.w;
