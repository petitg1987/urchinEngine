...
def move_handler(dummy):
    ...
    currentFiles = []
    for f in [f for f in os.listdir(file["directory"] + "/" + file["backup_directory"]) for c in
              range(1, int(file["backup_amount"]) + 1) if f == file["name"] + file["extension"] + str(c) if
              os.path.isfile(os.path.join(file["directory"] + "/" + file["backup_directory"], f))]:
    ...
    if len(currentFiles) < 1:  # If no files, then no need to check.
        if os.path.isfile(file["fullpath"] + "1"):
            shutil.move(file["fullpath"] + "1",
                        file["directory"] + "/" + file["backup_directory"] + "/" + file["name"] + ".blend1")
        ...
        if file["backup_amount"] <= len(currentFiles):
            ...
            for f in currentFiles:
                stats = os.stat(
                    file["directory"] + "/" + file["backup_directory"] + "/" + f)  # Get attributes from a file.
            ...
            shutil.move(file["fullpath"] + "1",
                        file["directory"] + "/" + file["backup_directory"] + "/" + replaceFile["fullname"])
            ...
            for f in currentFiles:
                for c in range(1, int(file["backup_amount"]) + 1):
                    if not os.path.isfile(
                            file["directory"] + "/" + file["backup_directory"] + "/" + file["name"] + file[
                                "extension"] + str(c)):
                        shutil.move(file["fullpath"] + "1",
                                    file["directory"] + "/" + file["backup_directory"] + "/" + file["name"] + file[
                                        "extension"] + str(c))
