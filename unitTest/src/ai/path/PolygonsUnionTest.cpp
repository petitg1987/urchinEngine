...
void PolygonsUnionTest::twoPolygonsUnionAxisAligned()
{
	...
	AssertHelper::assertPoint2FloatEquals(polygonUnion[0].getCwPoints()[3], Point2<float>(-2.0, 1.0)); //TODO wrong
	AssertHelper::assertPoint2FloatEquals(polygonUnion[0].getCwPoints()[4], Point2<float>(-2.0, 2.0)); //TODO wrong
	AssertHelper::assertPoint2FloatEquals(polygonUnion[0].getCwPoints()[5], Point2<float>(2.0, 2.0)); //TODO wrong
}
...
void PolygonsUnionTest::twoPolygonsSameEndPoint()
{
	...
	std::vector<CSGPolygon> allPolygons = {CSGPolygon("p1", polyPoints1), CSGPolygon("p2", polyPoints2)};
	std::vector<CSGPolygon> polygonUnion = PolygonsUnion().unionPolygons(allPolygons);
	AssertHelper::assertUnsignedInt(polygonUnion[0].getCwPoints().size(), 5); //TODO wrong
}
