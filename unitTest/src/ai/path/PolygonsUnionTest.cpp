...
void PolygonsUnionTest::onePolygonUnion()
{
	...
	std::vector<CSGPolygon> allPolygons = {CSGPolygon(polyPoints1)};
...
void PolygonsUnionTest::twoIdenticalPolygons()
{
	std::vector<Point2<float>> polyPoints1;
	polyPoints1.push_back(Point2<float>(0.0, 0.0));
	polyPoints1.push_back(Point2<float>(0.5, 1.0));
...
void PolygonsUnionTest::threePolygonsUnion()
{
	...
	std::vector<CSGPolygon> allPolygons = {CSGPolygon(polyPoints1), CSGPolygon(polyPoints2), CSGPolygon(polyPoints3)};
	std::vector<CSGPolygon> polygonUnion = PolygonsUnion().unionPolygons(allPolygons);
...
CppUnit::Test *PolygonsUnionTest::suite()
{
	...
	suite->addTest(new CppUnit::TestCaller<PolygonsUnionTest>("twoIdenticalPolygons", &PolygonsUnionTest::twoIdenticalPolygons));
